...
export function jsonToGo(json, typename, flatten = true,typeModel = 0) {
  let data;
  let scope;
  let go = "";
  let tabs = 0;
  ...
  function parseStruct(depth, innerTabs, scope, omitempty) {
    ...
    if (flatten && depth >= 2) {
      ...
      for (let i in keys) {
        ...
        //console.log(keys[i],goType(scope[keys[i]]),scope[keys[i]])
        if (typeModel == 1) {
          appender('" form:"' + keyname + '" xml:"' + keyname + '" binding:"required');
        }
        if (typeModel == 2) {
          const jdbcType = goJdbcType(goType(scope[keys[i]]))
          const column = toLine(keys[i])
          appender('" xorm:"'+jdbcType+' notnull unique \''+column+'\'');
        }
        appender('"`\n');
      }
      indenter(--innerTabs);
      ...
      for (let i in keys) {
        ...
        if (typeModel == 1) {
          append('" form:"' + keyname + '" xml:"' + keyname + '" binding:"required');
        }
        if (typeModel == 2) {
          const jdbcType = goJdbcType(goType(scope[keys[i]]))
          const column = toLine(keys[i])
          append('" xorm:"'+jdbcType+' notnull unique \''+column+'\'');
        }
        //console.log(keys[i],goType(scope[keys[i]]),scope[keys[i]])
        append('"`\n');
      }
...
//let c = jsonToGo('{"id":1,"productId":1001,"productName":"数字钱包","channelId":1001,"channelName":"马甲包新客户模型","suffix":"","function":"decision_tree","createAt":"2019-03-23T08:17:46.000+0000","useDecisionTree":1,"useScoreCard":1,"useDhbStatus":1,"useBlacklist":1,"useSdk":1,"useOnlineAnalyzer":0,"useTelVerification":0,"useTianChuangBankCard":0,"maxAmount":1}','',true,2)
//console.log(c.go)

//let cx = jsonToGo('{"userName":"x","user_info":{"userAge":0}}','',true,2)
//console.log(cx.go)
